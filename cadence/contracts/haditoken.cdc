...
pub contract haditoken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @haditoken.Vault
        ...
        // Destroy the vault
        destroy() {
            haditoken.totalSupply = haditoken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            haditoken.totalSupply = haditoken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&haditoken.Minter>(/public/Minter, target: /storage/MinterStorage)
